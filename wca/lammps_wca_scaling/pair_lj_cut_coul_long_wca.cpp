...
PairLJCutCoulLongWCA::PairLJCutCoulLongWCA(LAMMPS *lmp) : Pair(lmp)
{
...
PairLJCutCoulLongWCA::~PairLJCutCoulLongWCA()
{
  if (allocated) {
...
void PairLJCutCoulLongWCA::compute(int eflag, int vflag)
{
...
void PairLJCutCoulLongWCA::compute_inner()
{
...
void PairLJCutCoulLongWCA::compute_middle()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  double rsq,r2inv,r6inv,forcecoul,forcelj,factor_coul,factor_lj;
...
void PairLJCutCoulLongWCA::compute_outer(int eflag, int vflag)
{
...
void PairLJCutCoulLongWCA::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJCutCoulLongWCA::init_style()
{
  if (!atom->q_flag)
...
void PairLJCutCoulLongWCA::init_list(int id, NeighList *ptr)
{
  if (id == 0) list = ptr;
...
double PairLJCutCoulLongWCA::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  scaling_rcut[j][i] = scaling_rcut[i][j];
  scaling_rcutsq[j][i] = scaling_rcutsq[i][j];
  lj1[j][i] = lj1[i][j];
  lj2[j][i] = lj2[i][j];
  lj3[j][i] = lj3[i][j];
  lj4[j][i] = lj4[i][j];
...
void PairLJCutCoulLongWCA::write_restart(FILE *fp)
{
...
void PairLJCutCoulLongWCA::read_restart(FILE *fp)
{
...
void PairLJCutCoulLongWCA::write_restart_settings(FILE *fp)
{
...
void PairLJCutCoulLongWCA::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJCutCoulLongWCA::write_data(FILE *fp)
{
...
void PairLJCutCoulLongWCA::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
void *PairLJCutCoulLongWCA::extract(const char *str, int &dim)
{
  dim = 0;
